...
try {
  ...
  $title = isset($serverData['title']) ? $serverData['title'] : 'Máy Chủ Ngọc Rồng Online';
  $domain = validateURL($serverData['domain']);
  $_title = htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
  $_dailythe = htmlspecialchars($serverData['dailythe'], ENT_QUOTES, 'UTF-8');

  $_tenmaychu = isset($serverData['tenmaychu']) ? htmlspecialchars($serverData['tenmaychu'], ENT_QUOTES, 'UTF-8') : 'Máy Chủ Ngọc Rồng Online';
  $_token = $serverData['token'];

  $android = $serverData['android'];
  $iphone = $serverData['iphone'];
  $windows = $serverData['windows'];
  $java = $serverData['java'];
